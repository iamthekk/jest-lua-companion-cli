...
mod tests {
    ...
    fn test_sourcemap_creation_flat_format() {
        ...
        assert_eq!(sourcemap.get_entries().len(), 2);
    ...
    fn test_sourcemap_creation_tree_format() {
        ...
        // Check that path_map was created correctly
        assert_eq!(sourcemap.get_path_map().len(), 1);
    ...
    fn test_get_file_path() {
        ...
        sourcemap_with_lua.path_map.insert(
    ...
    fn test_convert_stack_trace() {
        ...
        // Test with @ in path (we'll just check it doesn't crash since path map is not accessible in test)
        assert!(true, "Path with @ handled without crashing");
    }
